...
def run_game(server_ip, port1, port2):
    ...
    try:
        while running:
            ...
            state_display = font.render(f"State: {state}", True, (200, 255, 128))
            screen.blit(state_display, (10, 10))
            msg_disp = font.render(msg_display, True, (255, 255, 255))
            screen.blit(msg_disp, (10, 400))

            if state == "WAIT_SHAKE":
                prompt = font.render("Please shake 5 times...", True, (255, 255, 0))
                screen.blit(prompt, (100, 220))
                with detector.lock:
                    if detector.ready:
                        state = "WAIT_SERVER"
                        msg_display = "waiting for server..."
                ...
                if data:
                    ...
                    if msg.startswith("PleaseAnswer"):
                        state = "ANSWER"
                        msg_display = "Please answer (A/B/C/D)"
                        reader.enabled = True
                        answer = None
                        # 顯示題目 (可依格式自動取得)
                ...
                if key in "abcd" and answer is None:
                    ...
                    msg_display = f"Your answer: {key.upper()}, Wait for checking..."
                ...
                if data:
                    ...
                    if msg.startswith("AnsweerCorrect"):
                        msg_display = "Correct！Go to next question"
                        state = "WAIT_SHAKE"
                        detector = ShakeDetector()
                        detector.start()
                    elif msg.startswith("AnswerWrong"):
                        msg_display = "Incorrect，Please shake again to get answer chance"
                        state = "WAIT_SHAKE"
                        detector = ShakeDetector()
                        detector.start()
                    elif msg.startswith("AnswerTimeOut"):
                        msg_display = "Timeout，Please shake again to get answer chance"
                        state = "WAIT_SHAKE"
                        detector = ShakeDetector()
                        detector.start()
                    elif msg.startswith("Gamefinished"):
                        msg_display = "Gameover！"
                        state = "END"
            elif state == "END":
                end_msg = font.render("press ESC to leave", True, (255, 50, 50))
